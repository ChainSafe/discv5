...
import { createPrivateKey, publicKey, sign, verify } from "./crypto";
import { ENR, ENRKey, ENRValue, NodeId, PrivateKey, SequenceNumber } from "./types";
...
export function encode(record: ENR, privateKey: PrivateKey, seq: SequenceNumber): Buffer {
  ...
  assert(encoded.length < constants.MAX_RECORD_SIZE, "ENR must be less than 300 bytes");
  return encoded;
}
