...
class SessionService extends EventEmitter {
    ...
    public async sendRequest(dstEnr: EthereumNodeRecord, msg: IMessage): Promise<void> {
      ...
      const dstPort = dstEnr.udp;
      const dstIpAddr = dstEnr.ipV4;
      const socketAddr: ISocketAddr = { dstPort, dstIpAddr};
      ...
      if ((session !== null) && !session.established()) {
        ...
        this.pendingMessages.set(dstNodeId, msgs);
          ...
          msgs.push(message);
          this.pendingMessages.set(dstNodeId, msgs);
          const {s, packet} = Session.newRandomSession(this.tag(dstNodeId), dstEnr);
          const r = newRequest(dstEnr, p, null);
          this.processRequest(dstNodeId, r);
   ...
   public async sendWhoAreYou(
     dstSocketAddr: ISocketAddr,
     nodeId: NodeId,
     enrSeq: bigint,
   ...
   public async flushMessages(dstSocketAddr: ISocketAddr, dstId: NodeId): Promise<void> {
     ...
     if (this.sessions.get(dstId).established()) {
       session = this.seesions.get(dstId);
     } else {
   ...
   public async checkTimeouts(): Promise<void> {
     for (const [nodeId, requests] of this.pendingRequests) {
        ...
        for (const [pos, req] of requests.entries()) {
        try {
