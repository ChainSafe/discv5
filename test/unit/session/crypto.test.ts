...
describe("session crypto", () => {
  it("ecdh should produce expected secret", () => {
    ...
    expect(Buffer.from(secp.getSharedSecret(Uint8Array.from(localSK), Uint8Array.from(remotePK), true))).to.deep.equal(
      expected
    );
  });
  ...
  it("key derivation should produce expected keys", () => {
    ...
    const secret = secp.getSharedSecret(Uint8Array.from(ephemKey), Uint8Array.from(destPubkey), true);
